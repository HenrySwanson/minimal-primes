...
use crate::sieve::sequence_slice::SequenceSlice;
use crate::sieve::stats::SieveStats;
...
pub fn sieve(
    ...
    let Some(n_range) = slices.iter().map(|slice| slice.range_len()).max() else {
...
mod tests {
    ...
    fn test_bsgs() {
        ...
        let num_remaining = slice.num_remaining();
        assert!(
            num_remaining < 20,
